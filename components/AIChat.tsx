...
const AIChat: React.FC = () => {
  ...
  const [messages, setMessages] = useState<{ role: 'user' | 'assistant'; content: string }[]>([
    { role: 'assistant', content: 'Olá! Sou a assistente virtual da Vivah Clínica. Como posso ajudar com suas dúvidas iniciais de saúde hoje?' }
  ]);
          ...
          <div ref={scrollRef} className="flex-1 p-6 overflow-y-auto space-y-4 bg-gray-50">
            {messages.map((m, i) => (
              <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[80%] p-4 text-xs font-medium leading-relaxed ${
                  m.role === 'user' 
